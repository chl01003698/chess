...
import SanZhangGamePlayer from './sanZhangGamePlayer';
import GamePlayer from '../../gamePlayer'
import FriendGame from '../../friendGame';
import { PokerAlgo, PokerCard } from '../pokerAlgo/pokerAlgo';
import { SanZhangAlgo } from '../pokerAlgo/sanzhang/sanzhangAlgo';
import SanZhuangBeat from '../pokerAlgo/sanzhang/sanZhangBeat';
import * as Raven from 'raven';
import Axios, * as axios from "axios"
export default class SanZhangGame extends FriendGame {
  zhuangId: string = ""
  baseScore: number = 1
  onceScore: number = 0;
  sumScore: number = 0        // 总筹码
  scores: Array<number> = []  //座子下注的筹码
  beatPlayers: Array<SanZhangGamePlayer> = []
  sitdownPlayers: Array<SanZhangGamePlayer> = []
  currBetRound: number = 0
  maxBetRound: number = 19
  timeout = 0
  addBets: Array<number> = []
  beatIds: Array<string> = []
  zhuangIndex: any = undefined;
  playerIds: any = [];
  LzCards: any = [];
  nextPlayer: any = "";  //下一个用户的情况
  wheelNum: any = 0;
  tmpzhuanIndex = 0;
  topPlayer: any = undefined;   //上一个用户 下注情况 及其是否是闷牌
  betArray: Array<number> = [];  //下注数量  每一轮下注数量
  chip: any = 1;
  isBrand: any = false;  //是否有人
  tmpZhuanPlayer: any = undefined;
  zhuanQiCards: any = false;
  private _sidIds: any = [];
  private _beatCards: any;
  qiongbi: any = false;
  isErSanWuEatBaoZi: any = false;
  sChip: number = 0;
  mChip: number = 0;
  sAddBets: any = [];
  mAddBets: any = [];
  ...
  private victoryPlayer: any = [];
  private isInitiateHuoPin: boolean = false;
  private DaXiPlayer: any = undefined;
  private peiCards: any = [];
  private chips: any = [];
  private biPaiPlayerList: any = [];
  private watchPlayerList: any = [];
  private watchArr: any = [];
  private huoPinPlayer: any = [];
  ...
  initFSM() {
    this.fsm = new StateMachine({
      ...
      init: "init",
      transitions: [
        { name: "readyTrans", from: ["over", "init"], to: "ready" }, //准备
        { name: "startTrans", from: "ready", to: "start" },     //游戏开始
        { name: "shuffleTrans", from: "start", to: "shuffle" },  //洗牌
        { name: "outputTrans", from: "shuffle", to: "output" },  //发牌
        { name: "inputTrans", from: ["output", "input"], to: "input" }, //加/跟/比
        { name: "overTrans", from: "input", to: "over" }, //游戏结束
        { name: "finishTrans", from: "*", to: "finish" },  //游戏大结算
  ...
  bindKeyMessage(gamePlayer: SanZhangGamePlayer, evetName: string, cb: any = undefined) {
  ...
  onEnterStart(lifecycle, arg1, arg2) {
    ...
    if ((this.roomConfig.wanfa != undefined && this.roomConfig.wanfa[0] == 1) ||
      ...
      this.cards = PokerAlgo.initCards(10, 15, false) //10--->A
    } else {
      this.cards = PokerAlgo.initCards(2, 15, false) // 2--->A
    }
    ...
    this._beatCards = new SanZhuangBeat(this);
    let playerInfos: any = [];
    let _daxiPlayer: any = undefined;
    if (this.DaXiPlayer != undefined) _daxiPlayer = _.find(this.getAllGamePlayers(), { uid: this.DaXiPlayer.uid });
    let flag = false;
    ...
    _.forEach(this.beatPlayers, (v, i) => {
      ...
      if (this.zhuangIndex != undefined) {
        if (zhuangFlag == false) {
          if (v.index == this.zhuangIndex) zhuangFlag = true;
        }
      }
    ...
    if (this.zhuangIndex == undefined || zhuangFlag == false) this.zhuangIndex = this._sidIds[0];
    ...
    if (this.roomConfig["lanzi"] != undefined && this.roomConfig["lanzi"][0] == 1) {
      let cards = PokerAlgo.pickCardIds(this.cards);
      this.LzCards = SanZhangAlgo.getLzCards(cards);
      // this.LzCards = ["0.14"];
      let tmpCards = this.LzCards.slice();
      _msg["LzCards"] = SanZhangAlgo.getPlayerCards(tmpCards, tmpCards[0])
      // _msg["LzCards"] = this.LzCards;  //懒子牌
    }
    this.DaXiPlayer = undefined;
    this.container.channel.pushMessage(pushEvent.sz_onGameStart, _msg);
    process.nextTick(() => {
      this.fsm.shuffleTrans()
    })
  }
  onEnterOutput(lifecycle, arg1, arg2) {
    let temCards = PokerAlgo.pickCardIds(this.cards);
    // let num = _.random(2, 3);
    let _cards = this.clearCards(temCards);
    this.cards = PokerAlgo.createCardsFromIds(_cards);
    const dealCards = PokerAlgo.dealCards(this.cards as Array<PokerCard>, 3)
    let sid: any = 0;
    if (dealCards.length > 0 && this.sitdownPlayers.length <= dealCards.length) {
      let cards: any = []
      _.forEach(this.sitdownPlayers, (v, i) => {
        cards = PokerAlgo.pickCardIds(dealCards[i]);
        if (this.roomConfig["lanzi"] != undefined && this.roomConfig["lanzi"][0] == 1) cards = SanZhangAlgo.getPlayerCards(cards, this.LzCards[0]);
        if (v.cards.length == 0) v.cards = this._beatCards.sortCardsPlayer(cards)
        this.container.channel.pushMessageByIds(pushEvent.sz_onOutput, { cards: ["-1", "-1", "-1"], sid: v.index, uid: v.uid, addBets: this.addBets }, v.uid)
      })
      process.nextTick(() => {
        this.fsm.inputTrans()
      })
  ...
  clearCards(temCards: Array<string>) {
    let clearCards = [];
    // let num = _.random(2, 3);
  ...
  onEnterInput(lifecycle, gamePlayer, msg) {
    ...
    if (gamePlayer != undefined && msg != undefined) {
      gamePlayer.action = {};
      if (gamePlayer.isSpeck == false) return true;
      ...
      if (type == 0) {  //跟注
        ...
        let targetGamePlayer: SanZhangGamePlayer = _.find(this.beatPlayers, { uid: param }) //和这个player比牌
        ...
        if (this.beatPlayers[0].uid == gamePlayer.uid) this.beatPlayers[0] = gamePlayer;
        if (this.beatPlayers[1].uid == gamePlayer.uid) this.beatPlayers[1] = gamePlayer;
        ...
        gamePlayer.chip = gamePlayer.chip - this.chip;
        this.sumScore = this.sumScore + this.chip;
        if (this.isInitiateHuoPin == true) { this.isInitiateHuoPin = false } else { this.isInitiateHuoPin = true };
        gamePlayer.addBets.push(this.chip);
        this.pushRadio(gamePlayer, "huopin", type);
        if (gamePlayer_1.huopin == true && gamePlayer_2.huopin == true) { //火拼
  ...
  qunBi() {
    ...
    this.container.channel.pushMessage(pushEvent.sz_onQunBi, { results: result, "kanPaiList": kanPaiList, state: "qunbi" });
    for (let i = 0; i < this.beatPlayers.length; i++) {
  ...
  thanCard(gamePlayer, targetGamePlayer, type) {
      ...
      for (let i = 0; i > this.gamePlayers.length; i++) {
        ...
        if (_uid == successPlayer.uid) {
          if (successPlayer.showCards == true) {
            this.container.channel.pushMessageByIds(pushEvent.sz_onBiPai, obj, _uid);
    ...
    this.container.channel.pushMessage(pushEvent.sz_onBiPai, obj)
  ...
  //广播协议
  pushRadio(gamePlayer: any, state: any, type: any, playerIds: any = false, flagType = false) {
    let chip = this.chip;
    let chips: any = [];
    for (let i = 0; i < this.beatPlayers.length; i++) {
    ...
    if (type == 5 || type == 6) delete _obj["chip"];
    if (type == 5) if (gamePlayer.showCards == false) _obj["liangpai"] = 1;
    if (_obj["chip"] != undefined) this.chips.push(this.chip);
    this.container.channel.pushMessage(pushEvent.sz_onRadio, _obj); //推送跟组广播
  }
  checkGameOver() {
  ...
  timing(fun, times: any, gameOver = false) {
    let self = this;
    if (gameOver == true) this.container.channel.pushMessage(pushEvent.sz_onJieSuan, { "jieSuan": true });
    setTimeout(function () {
      fun(false, false, self);
    }, times);
  }
  //加注
  bet(gamePlayer: any, type: any, index: any) {
  ...
  setWheelRound(sid) {
    if (this.zhuanQiCards == false) if (sid == this.tmpzhuanIndex) this.wheelNum += 1;
  ...
  //获取筹码
  getAddBets(betNum: number, flag: boolean = false) {
    //获取第二个人的筹码
  ...
  //跟注
  follow(gamePlayer: any, type: any) {
    // this.checkMenPai();
    gamePlayer.isBiPai = false;
    if (gamePlayer.showCards == false) gamePlayer.menpai += 1;
    if (this.topPlayer == undefined) {  //第一个用户说话
      if (gamePlayer.showCards == true) this.chip = this.chip * 2;
      gamePlayer.betNum = this.chip;
    } else {
      if (this.topPlayer.showCards == true) { //上一个玩家看牌 压得
        if (this.isBrand == true) this.chip = this.mChip; //Math.ceil(this.sChip / 2); //这个玩家 有人闷牌 
        if (gamePlayer.showCards == true) this.chip = this.sChip; // this.topPlayer.betNum;  //两个玩家都看牌了
      }
      if (this.topPlayer.showCards == false) {  //上一个玩家没有看牌  
        if (this.isBrand == true) this.chip = this.mChip; //this.topPlayer.betNum;
        if (gamePlayer.showCards == true) this.chip = this.sChip;//this.topPlayer.betNum * 2;
      }
      gamePlayer.betNum = this.chip;
      this.topPlayer = gamePlayer;
    }
  ...
  checkMenPai() {
    ...
    if (player != undefined) this.isBrand = true;
    let mChip = 1;
    let sChip = 2
    for (let i = 0; i < this.beatPlayers.length; i++) {
      if (this.beatPlayers[i].showCards == true) {
        if (this.beatPlayers[i].betNum > sChip) sChip = this.beatPlayers[i].betNum;
      }
      if (this.beatPlayers[i].showCards == false) {
        if (this.beatPlayers[i].betNum > mChip) mChip = this.beatPlayers[i].betNum;
      }
    }
    this.sChip = sChip;
    this.mChip = mChip;
    if (this.mChip * 2 > this.sChip) this.sChip = mChip * 2;
    if (this.sChip / 2 > this.mChip) this.mChip = sChip / 2;
  }
  getPlayerAction(isQiPai: any = false, gamePlayer: any = false, self = this, isHuoPin = false) {
    self.turnNext();
    self.checkMenPai();
    if (self.nextPlayer) {
    ...
    let kanPai: Array<any> = [];
    if (isQiPai == true) {
      if (gamePlayer.index == self.tmpzhuanIndex) { //庄家弃牌  开始移到下一个庄家  到庄家一轮结束
        self.tmpzhuanIndex = self.nextPlayer.index;
      }
    }
    if (self.nextPlayer.showCards == true) self.addBets = self.getAddBets(self.sChip, self.nextPlayer.showCards);
    if (self.nextPlayer.showCards == false) self.addBets = self.getAddBets(self.mChip, self.nextPlayer.showCards);
    let _msg: object = {
      "jiazhu": 1,//加注
      "genzhu": 1, //跟注
      speakUid: self.nextPlayer.uid,
      speakSid: self.nextPlayer.index,
      wheelNum: self.wheelNum, //轮数
    }
    if (self.wheelNum >= 1) _msg["bipai"] = 1;
    _msg["addBets"] = self.addBets
    if (self.addBets.length == 0) delete _msg["jiazhu"];

    if (self.roomConfig["zhongtu"] != undefined && self.roomConfig["zhongtu"][0] == 1) {
      if (this.beatPlayers.length > 2) delete _msg["bipai"];
    }
    ...
    for (let i = 0; i < self.gamePlayers.length; i++) {
      if (self.gamePlayers[i].watcher == false) {
        if (self.gamePlayers[i].showCards == false && self.gamePlayers[i].giveup == false) kanPai.push({ sid: self.gamePlayers[i].index, uid: self.gamePlayers[i].uid });
      }
    }

    _msg["kanPaiList"] = kanPai;
    if (self.roomConfig["mensanlun"] != undefined && self.roomConfig["mensanlun"][0] == 1) { //闷三轮 无看牌选项
      if (self.wheelNum < 3) {
        delete _msg["kanpai"];
    ...
    if (self.nextPlayer.showCards == true) delete _msg["kanpai"];
    ...
    self.container.channel.pushMessage(pushEvent.sz_onFirstInput, _msg)
    return _msg;
  }
  checkIsBiPai() {
    for (let i = 0; i < this.beatPlayers.length; i++) {
      if (this.beatPlayers[i].isBiPai == true) return false;
    }
    return true;
  }
  //弃牌
  removeBeatPlayer(beatPlayer: SanZhangGamePlayer, status: boolean = false) {
    ...
    if (beatPlayer.index == this.tmpzhuanIndex) {//&& beatPlayer.index == this.nextPlayer.index) {
      ...
      if (status == true) this.timing(this.getPlayerAction, 3000);
    }
    ...
    if (status != true) {
      ...
      if (this.beatPlayers.length == 2) { //只剩下两个人
        ...
        for (let i = 0; i < this.gamePlayers.length; i++) {
          if (this.gamePlayers[i].watcher == false) {
            if (this.gamePlayers[i].showCards == false && this.gamePlayers[i].giveup == false) kanPaiList.push({ sid: this.gamePlayers[i].index, uid: this.gamePlayers[i].uid });
          }
        }
        _msg["kanPaiList"] = kanPaiList;
        this.nextPlayer.action = _msg;
        this.container.channel.pushMessage(pushEvent.sz_onFirstInput, _msg)
      }
    }
    return false;
  ...
  leave(userId: string, session: any) {
    ...
    if (!player) this.disPlayer.push(gamePlayer);
    if (player) player.leave = true;
    ...
    this.container.channel.pushMessage(pushEvent.sz_onLeave, { leave: obj }); //通知离开
  ...
  turnZhuanNext() {
    ...
    while (true) {
      ...
      if (nextPlayer == undefined) continue;
  ...
  zhuangPlayer(flag: boolean = false) {
    let ObjSid: any = []
    let sid: number = 0;
    for (let i = 0; i < this.beatPlayers.length; i++) {
  ...
  peiPai(uid: any, msg: object) {
    let player = this.findPlayerByUid(uid);
    if (_.isArray(msg["type"])) {
      if (this.LzCards.length > 0) {
        msg["type"] = SanZhangAlgo.getPlayerCards(msg["type"], this.LzCards[0]);
      }
      player.cards = msg["type"];

    } else {
      if (this.gameConfig["peipai"][msg["type"]]) player.cards = this.gameConfig["peipai"][msg["type"]];
    }
    return true;
  }
  //看牌
  lookCards(uid) {
    ...
    if (action["kanpai"] != undefined) delete action["kanpai"];
    player.action = action;
    if (this.beatPlayers.length == 2) action["huoping"] = 1; //有火拼
    ...
    this.container.channel.pushMessageByIds(pushEvent.sz_onShowCards_, { cards: player.cards, action: action, type: player.type, isRuan: player.isRuan }, uid);
    // this.pushMsg(uid,{ cards: player.cards, action: action },pushEvent.sz_onShowCards_);

  }
  get_CardsType(player: any) {
    if (this.roomConfig["lanzi"] != undefined && this.roomConfig["lanzi"][0] == 1) {
      ...
      if (cardsObj["LzCards"]) {
        ...
        if (player.type == "baozi") {
          ...
          if (flag == false) player.isRuan = false;
        }
        return true;
      }
    }
    let cards = PokerAlgo.createCardsFromIds(player.cards);
    let info: object = this._beatCards.getCheckCards(cards, player);
  ...
  showCards(uid) {
    ...
    this.get_CardsType(player);
    this.container.channel.pushMessage(pushEvent.sz_onShowCards, { cards: player.cards, sid: player.index, uid: player.uid, type: player.type, isRuan: player.isRuan });
  ...
  getBiPaiPlayers(uid) {
    ...
    this.container.channel.pushMessageByIds(pushEvent.sz_onBiPaiPlayers_, { players: players }, uid);
  }
  getBetInfo() {
  ...
  onBeforeOverTrans(lifecycle, arg1, arg2) {
    if (this.roomConfig.paicai == true && this.beatPlayers.length == 1) {
      const gamePlayer: any = this.beatPlayers[0]
      const [type, multiple] = SanZhangAlgo.getPaiCaiResult(gamePlayer.cards)
      if (type >= 0) {
        let sumScore = 0
        let score = this.baseScore * multiple
        _.forEach(this.sitdownPlayers, (v: GamePlayer) => {
        ...
        this.container.channel.pushMessage(pushEvent.sz_onPaiCai, {
          uid: gamePlayer.uid,
          cards: PokerAlgo.pickCardIds(gamePlayer.cards),
          score: sumScore,
          type: type,
          multiple: multiple
        })
  ...
  onEnterOver(lifecycle, arg1, arg2) {
    ...
    for (let i = 0; i < players.length; i++) {
      ...
      if (_player.watcher != true) {
        ...
        if (_player.giveup == true || _player.thanCards == true || _player.leave == true) {  //弃牌
    ...
    this.container.channel.pushMessage(pushEvent.sz_onGameResult, {
      "results":
        results, currentRound: this.currentRound, roundCount: this.roomConfig.roundCount
    });
  ...
  getResults(uid) {
    this.container.channel.pushMessageByIds(pushEvent.sz_onResults, { results: this.gameResults, round: this.currentRound }, uid);
  }
  saveCurrentRoundGamePlayer(roundObject: object, GamePlayer: GamePlayer, index: number) {
    if (index >= 0) if (this._results) _.assign(roundObject, this._results[index]);
  }
  clientInfo(uid?: string) {
    let clientInfo = super.clientInfo(uid);
    // if(this.isStart == true) clientInfo["watcher"] = true;
    if (this.container.countDown.existTime("ready")) clientInfo["redayTimes"] = this.container.countDown.getRemainingDuration("ready");
    ...
    if (uid && this.isStart == true && this.fsm.is('ready') != true) {
      if (player.cards.length == 0) player.watcher = true;
      ...
      for (let i = 0; i < players.length; i++) {
        ...
        if (Object.keys(_player.action).length != 0) {
          ...
          if (_player.action["genzhu"] == undefined) if (_player.action["huopin"] != undefined) clientInfo["isHuoPin"] = true;
        }

        if (_player.cards.length == 0) obj["watcher"] = true;
        playerInfos.push(obj);
      }
      ...
      if (this.LzCards.length > 0) {
        let tmpCards = this.LzCards.slice();
        clientInfo["LzCards"] = SanZhangAlgo.getPlayerCards(tmpCards, tmpCards[0])
      }
  ...
  getPlayerType() {
    return SanZhangGamePlayer
  }
  resortBeatPlayers(nextIndex: number) {
    if (nextIndex > 0) {
