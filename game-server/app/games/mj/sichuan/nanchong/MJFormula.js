...
class MJFormula {
    static getConfigScore(name, game, mjscoreManage, index) {
        return MJFormulaModel_1.ScoreDataBase.getComboScoreByName(name, game, mjscoreManage, index);
    }
    ...
    static getScoreV2(params) {
        ...
        _.forEach(gamePlayers, (v) => {
            scoreInfo[v.uid] = new MJFormulaModel_1.ScoreData();
        });
    ...
    static getPreScore(params) {
        ...
        // 杠分
        let gangScore = MJFormulaModel_1.ScoreDataBase.getGang(game, mjscoreManage, player.index);
        // 牌型分
